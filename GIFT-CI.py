...
class GIFT_CI(nn.Module):
    def __init__(self, model_cfg: Dict,num_classes: int = 1000):
        ...
        self.linear1 = nn.Linear(800, 1280)
        self.linear2 = nn.Linear(1280, 640)
        self.linear3 = nn.Linear(640, 320)
        self.last_linear4 = nn.Linear(in_features=320, out_features=num_classes)
    ...
    def forward(self, a: Tensor, v: Tensor, cf: Tensor) -> Tensor:
        #route 1
        a = self.conv_1_a(a)
        a = self.layer_1_a(a)
        a = self.layer_2_a(a)
        ...
        av_cf = torch.cat((av,cf), dim=1)

        output = self.linear1(av_cf)
...
def GIFT_CI_(num_classes: int = 1000):
    config = get_config("small")
    m = GIFT_CI(config, num_classes=num_classes)
    return m
