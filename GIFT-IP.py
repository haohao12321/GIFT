...
class GIFT_IP(nn.Module):
    ...
    def forward(self, a: Tensor, v: Tensor, az: Tensor,vz: Tensor) -> Tensor:
        ...
        combined_output = torch.cat((av, avz), dim=1)
...
def GIFT_IP_(num_classes: int = 1000):
    config = get_config("small")
    m = GIFT_IP(config, num_classes=num_classes)
    return m
